...
/**
 * BrainAnatomy — Gray's Anatomy lateral-view brain (Plate 728, 1918, public domain).
 * Vectorised by Mysid (Wikimedia Commons File:Brain_diagram_without_text.svg).
 * Anatomical lobes recoloured to map functional regions of the agentic loop.
 *
 *   Frontal lobe (front-left)   → Thinking  (orange)
 *   Parietal lobe (top)         → Deciding  (gold)
 *   Temporal lobe (lower-mid)   → Memory    (indigo)
 *   Cerebellum (back-bottom)    → Creating  (lime)
 *   Occipital lobe + brain stem → Neutral fill
 */

const BRAIN_INNER_SVG = `<path fill="#fcfb98" d="M564 54c13-2 26-1 38 6 38-7 66-1 83 18 14-3 29 1 43 11 39 17 67 39 89 64 36 15 50 33 52 53 19 14 31 31 39 50L798 413l-343-43-103-161L564 54z"/>
    ...
    <path fill="#8a686e" fill-opacity=".60512821" stroke="#000" stroke-width="2" d="M518 579c7 28 20 47 41 55 30 14 53 48 77 78 13-7 27-13 38-23L560 553l-42 26z" class="fil2 str0"/>
...
// Original brain viewBox is 1024×732; lobe centres in that coordinate space.
const ANCHORS = {
  thinking:  { x: 240, y: 270 },  // frontal lobe (front-left)
  deciding:  { x: 600, y: 200 },  // parietal lobe (top)
  memory:    { x: 360, y: 430 },  // temporal lobe (lower-mid)
  creating:  { x: 720, y: 595 },  // cerebellum (back-bottom)
}
...
export default function BrainAnatomy({ showLabels = false, showSynapses = false, pulseHighlight = false }: Props) {
  const T = ANCHORS.thinking
  const D = ANCHORS.deciding
  const M = ANCHORS.memory
  const C = ANCHORS.creating

  return (
    <svg
      viewBox="20 20 990 660"
      className="brain-anatomy"
      xmlns="http://www.w3.org/2000/svg"
      role="img"
      aria-label="A lateral view of the brain with four functional regions highlighted: frontal cortex (thinking), parietal lobe (deciding), temporal lobe (memory) and cerebellum (creating)."
    >
      <defs>
        <radialGradient id="brain-bg-glow-2" cx="50%" cy="55%" r="60%">
          <stop offset="0%" stopColor="#ffffff" stopOpacity="0.06" />
          <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
        </radialGradient>
        <filter id="brain-region-glow" x="-30%" y="-30%" width="160%" height="160%">
          <feGaussianBlur stdDeviation="6" />
        </filter>
      </defs>

      {/* Soft aura behind the brain */}
      <ellipse cx="515" cy="400" rx="530" ry="380" fill="url(#brain-bg-glow-2)" />

      {/* Original Gray's anatomy SVG, recoloured via CSS attribute selectors */}
      <g className="brain-anatomy-lobes" dangerouslySetInnerHTML={{ __html: BRAIN_INNER_SVG }} />

      {/* Animated synapses forming the agentic loop */}
      {showSynapses && (
        <g className="brain-anatomy-synapses">
          <path className="brain-synapse" stroke="#f46c42" d={`M ${T.x} ${T.y} Q 420 150 ${D.x} ${D.y}`} />
          <path className="brain-synapse" stroke="#c59f4d" d={`M ${D.x} ${D.y} Q 720 380 ${C.x} ${C.y}`} style={{ animationDelay: '-1.5s' }} />
          <path className="brain-synapse" stroke="#d1f503" d={`M ${C.x} ${C.y} Q 540 540 ${M.x} ${M.y}`} style={{ animationDelay: '-3s' }} />
          <path className="brain-synapse" stroke="#7902f3" d={`M ${M.x} ${M.y} Q 280 360 ${T.x} ${T.y}`} style={{ animationDelay: '-4.5s' }} />
        </g>
      )}

      {/* Glowing nodes at each region centre */}
      <g className="brain-anatomy-nodes">
        <circle className="brain-node brain-node-thinking" cx={T.x} cy={T.y} r="9" />
        <circle className="brain-node brain-node-deciding" cx={D.x} cy={D.y} r="9" />
        <circle className="brain-node brain-node-memory"   cx={M.x} cy={M.y} r="9" />
        <circle className="brain-node brain-node-creating" cx={C.x} cy={C.y} r="9" />
      </g>

      {/* Pulse rings (mapping slide) */}
      {pulseHighlight && (
        <g>
          <circle cx={T.x} cy={T.y} r="22" fill="none" stroke="#f46c42" strokeWidth="1.5" opacity="0.6">
            <animate attributeName="r" from="14" to="64" dur="2.4s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.7" to="0" dur="2.4s" repeatCount="indefinite" />
          </circle>
          <circle cx={D.x} cy={D.y} r="22" fill="none" stroke="#c59f4d" strokeWidth="1.5" opacity="0.6">
            <animate attributeName="r" from="14" to="64" dur="2.4s" begin="0.6s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.7" to="0" dur="2.4s" begin="0.6s" repeatCount="indefinite" />
          </circle>
          <circle cx={C.x} cy={C.y} r="22" fill="none" stroke="#d1f503" strokeWidth="1.5" opacity="0.6">
            <animate attributeName="r" from="14" to="64" dur="2.4s" begin="1.2s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.7" to="0" dur="2.4s" begin="1.2s" repeatCount="indefinite" />
          </circle>
          <circle cx={M.x} cy={M.y} r="22" fill="none" stroke="#7902f3" strokeWidth="1.5" opacity="0.6">
            <animate attributeName="r" from="14" to="64" dur="2.4s" begin="1.8s" repeatCount="indefinite" />
            <animate attributeName="opacity" from="0.7" to="0" dur="2.4s" begin="1.8s" repeatCount="indefinite" />
          </circle>
        </g>
      )}
      ...
      {/* Anatomical labels */}
      {showLabels && (
        <g className="brain-anatomy-labels">
          {/* Thinking — leader to bottom-left */}
          <line className="brain-leader brain-leader-thinking" x1={T.x - 20} y1={T.y + 30} x2="60" y2="640" />
          <text x="30" y="625" className="brain-label-tag">Frontal cortex</text>
          <text x="30" y="660" className="brain-label">Thinking</text>

          {/* Deciding — leader to top */}
          <line className="brain-leader brain-leader-deciding" x1={D.x} y1={D.y - 30} x2={D.x + 50} y2="60" />
          <text x={D.x + 60} y="48" className="brain-label-tag">Parietal lobe</text>
          <text x={D.x + 60} y="80" className="brain-label">Deciding</text>

          {/* Memory — leader to top-left */}
          <line className="brain-leader brain-leader-memory" x1={M.x - 30} y1={M.y} x2="60" y2="60" />
          <text x="30" y="48" className="brain-label-tag">Temporal lobe</text>
          <text x="30" y="80" className="brain-label">Memory</text>

          {/* Creating — leader to bottom-right */}
          <line className="brain-leader brain-leader-creating" x1={C.x + 30} y1={C.y - 10} x2="970" y2="640" />
          <text x="970" y="625" className="brain-label-tag" textAnchor="end">Cerebellum</text>
          <text x="970" y="660" className="brain-label" textAnchor="end">Creating</text>
        </g>
      )}
    </svg>
  )
